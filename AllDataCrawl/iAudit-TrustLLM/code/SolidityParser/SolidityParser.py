...
def getFunction(i, stream, max_length, function_id, start, outside_contract):
    ...
    while i < max_length:
        ...
        if id is not None:
            ...
            if (left_bracket_count != 0) and (
                left_bracket_count == right_bracket_count
            ):
                break
        i += 1
    if loc is None:
    ...
    while i < max_length:
        ...
        if id is not None:
            ...
            if (CLONE_TYPE == 2) and (id == VARIABLE_NAME_ID):
                ...
                i, function, function_entry = getFunction(
                    i + 1, stream, max_length, id, loc, False
                )
...
def parseStream(stream: CommonTokenStream):
    try:
        ...
        while i < length:
            ...
            if id is not None:
                if id == PRAGMA_ID:
                    i, compiler_, _, content = forwardUntil(
                        i + 1, stream, COMPILER_VERSION_ID, length
                    )
                    original_output += content
                    compiler = compiler_.replace("solidity", "")
                elif id == IMPORT_ID:
                    i, import_statement, _, content = forwardUntil(
                        i + 1, stream, IMPORT_STATEMENT_ID, length
                    )
                    original_output += content
                    ...
                    i, content, subcontract = getSubcontract(
                        i + 1, stream, length, id, loc
                    )
                    ...
                    i, content, function = getFunction(
                        i + 1, stream, length, id, loc, True
                    )
